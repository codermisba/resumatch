...
# Backend URL
# ---------------------------
BACKEND_URL = "http://127.0.0.1:8000"  # Change this to deployed backend URL if needed
...
st.markdown("""
    ...
    .stForm { background-color: #ffffff; padding: 20px; border-radius: 12px; box-shadow: 0 6px 12px rgba(0,0,0,0.1); margin-bottom: 25px; }
    label { font-weight: bold; color: #495057; }
    div.stFileUploader { border: 2px dashed #0a6ebd; border-radius: 8px; padding: 15px; background-color: #e9f2fb; }
    div.stButton > button { background-color: #0a6ebd; color: white; font-weight: bold; padding: 0.5em 1.2em; border-radius: 8px; border: none; }
    div.stButton > button:hover { background-color: #0857a6; color: white; }
    .metric-label { font-weight: 600; font-size: 16px; color: #495057; }
    .metric-value { font-size: 32px; color: #0a6ebd; font-weight: bold; }
    ...
    .dataframe-container { background-color: #ffffff; padding: 15px; border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); margin-top: 15px; }
    </style>
        ...
        if not candidate or not job_id or not resume or not jd:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.write(", ".join(result["missing"]) if result["missing"] else "No major gaps detected.")
                else:
